...
export function BalancePill() {
  ...
  const { tokenBalance } = session;
  const dailyLeft = Math.max(session.spendingLimits.daily - session.spendingUsage.today, 0);
  const monthlyLeft = Math.max(session.spendingLimits.monthly - session.spendingUsage.month, 0);

  return (
    <Tooltip
      label={`Límite diario restante: ${formatTokens(dailyLeft)} · Mensual: ${formatTokens(monthlyLeft)}`}
      className="w-full"
    >
