  ...
  return (
    <section className="w-full max-w-xl rounded-[2rem] border border-slate-200 bg-white p-6 shadow-2xl sm:p-8">
      ...
      <form className="mt-6 space-y-4" onSubmit={handleSubmit}>
        {mode === "register" && <div><label className="text-sm font-bold text-slate-700" htmlFor="fullName">Nombre completo</label><input className="mt-2 w-full rounded-2xl border border-slate-200 px-4 py-3 outline-blue-500" id="fullName" onChange={(event) => setFullName(event.target.value)} placeholder="Nombre Apellidos" value={fullName} /></div>}
