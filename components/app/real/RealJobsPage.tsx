...
export function RealJobsPage() {
  ...
  async function openOriginal(job: JobRecord) {
    ...
    const response = await fetch(`/api/jobs/${job.id}/original/signed-url`);
    const result = await response.json().catch(() => ({}));
    if (!response.ok || !result.signedUrl) return showToast(result.error ?? "No se pudo crear signed URL del original.", "error");
    window.open(result.signedUrl, "_blank", "noopener,noreferrer");
    showToast("Signed URL privada creada para el archivo original.", "success");
  }
