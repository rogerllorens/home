...
export function RealDownloadsPage() {
  ...
  return <div className="space-y-8"><div className="grid gap-4 md:grid-cols-3"><Card><p className="text-sm text-slate-500">Descargas reales</p><p className="mt-2 text-3xl font-black">{downloads.length}</p></Card><Card><p className="text-sm text-slate-500">CSV</p><p className="mt-2 text-3xl font-black">{downloads.filter((d) => d.file_type.startsWith("csv")).length}</p></Card><Card><p className="text-sm text-slate-500">Informes</p><p className="mt-2 text-3xl font-black">{downloads.filter((d) => d.file_type.includes("report")).length}</p></Card></div>{downloads.length ? <Card><Table headers={["Archivo", "Tipo", "Filas", "Score", "Productos", "Fecha", "Acción"]}>{downloads.map((download) => <tr key={download.id}><Td><b>{download.filename}</b></Td><Td><Badge variant="info">{download.file_type}</Badge></Td><Td>{download.rows_count}</Td><Td>{download.average_score ?? "—"}</Td><Td>{download.product_equivalent_used ?? "—"}</Td><Td>{new Date(download.created_at).toLocaleDateString("es-ES")}</Td><Td><Button onClick={() => open(download)} variant="secondary">Descargar</Button></Td></tr>)}</Table></Card> : <EmptyState title="Todavía no tienes descargas reales" description="Cuando el worker procese tu CSV aparecerán aquí los outputs con signed URLs privadas." icon="↓" action={<Button href="/app/upload">Subir CSV</Button>} />}</div>;
}
