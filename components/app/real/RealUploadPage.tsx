...
import { parseImportPreview, parsePastedTable, importedRowsToCsv, type ImportedCatalog } from "@/lib/import";
import { MappingConfidencePanel } from "@/components/app/import/MappingConfidencePanel";
import { SpreadsheetSheetSelector } from "@/components/app/import/SpreadsheetSheetSelector";
import { BulkCorrectionPanel, type EditableImportRow } from "@/components/app/import/BulkCorrectionPanel";
import { TemplateDownloadsPanel } from "@/components/app/import/TemplateDownloadsPanel";
...
export function RealUploadPage() {
  ...
  const [importCatalog, setImportCatalog] = useState<ImportedCatalog | null>(null);
  ...
  const auditToken = typeof window !== "undefined" ? new URLSearchParams(window.location.search).get("auditToken") : null;
  ...
  const limitWarning = useMemo(() => rows.length > maxRowsPerJob ? `Tu configuración procesa hasta ${maxRowsPerJob.toLocaleString("es-ES")} filas por job. Divide el catálogo en varios lotes.` : null, [rows.length, maxRowsPerJob]);

  function mappingFromCatalog(catalog: ImportedCatalog): ColumnMapping { const byField = Object.fromEntries(catalog.mappingSuggestions.map((s) => [s.field, s.selectedColumn])); return { sku: byField.sku ?? undefined, nombre_producto: byField.product_name ?? undefined, marca: byField.brand ?? undefined, categoria: byField.category ?? undefined, descripcion_actual: byField.description ?? byField.short_description ?? undefined, keyword_principal: byField.keywords ?? undefined, keywords_secundarias: byField.keywords ?? undefined, plataforma: undefined, precio: byField.price ?? undefined, url_actual: byField.product_url ?? byField.handle ?? undefined, imagen_url: byField.image_url ?? undefined }; }

  function analyzeImportedCatalog(catalog: ImportedCatalog, name = fileName) { const csv = importedRowsToCsv(catalog.headers, catalog.rows); const mapped = mappingFromCatalog(catalog); setImportCatalog(catalog); analyzeText(csv, name, mapped); }

  function applyBulkRows(nextRows: EditableImportRow[]) { const csv = [headers.join(","), ...nextRows.map((row) => headers.map((header) => `"${String(row[header] ?? "").replace(/"/g, '""')}"`).join(","))].join("\n"); analyzeText(csv, fileName, mapping); }

  async function selectWorkbookSheet(sheetId: string) { if (!fileBuffer || !file) return; setBusy("reading"); try { const catalog = await parseImportPreview({ buffer: fileBuffer, fileName: file.name, mime: file.type, sheetId }); analyzeImportedCatalog(catalog, `${file.name}.normalized.csv`); showToast(`Hoja ${catalog.sheetName ?? sheetId} seleccionada y remapeada.`, "success"); } catch (error) { showToast(error instanceof Error ? error.message : "No hemos podido leer la hoja seleccionada.", "error"); } finally { setBusy("idle"); } }

  function analyzeText(text: string, name = fileName, forcedMapping?: ColumnMapping) {
    setBusy("analyzing");
    try {
      ...
      if (parsed.headers.length > 80) showToast("Catálogo con muchas columnas. Revisa el mapeo antes de crear job.", "warning");
      const autoMapping = forcedMapping ?? autoMapColumns(parsed.headers);
      ...
      setAnalysis(summary);
      showToast(`Catálogo analizado: ${parsed.rows.length} filas y ${parsed.headers.length} columnas.`, "success");
      setAiPreview([]);
  ...
  async function handleFile(event: ChangeEvent<HTMLInputElement>) {
    ...
    if (!/\.(csv|tsv|xlsx|xls|xml)$/i.test(selected.name)) {
      showToast("Formato no permitido. Sube CSV, TSV, XLSX, XLS o XML básico.", "error");
      return;
    }
    if (selected.size > maxFileSize) {
      showToast("El archivo supera 10MB. Divide el catálogo antes de importarlo.", "error");
      return;
    }
    setBusy("reading");
    try {
      ...
      const catalog = await parseImportPreview({ buffer, fileName: selected.name, mime: selected.type });
      analyzeImportedCatalog(catalog, `${selected.name}.normalized.csv`);
      showToast(`${catalog.sourceType.toUpperCase()} importado y convertido a CSV interno UTF-8.`, "success");
    } catch (error) {
      showToast(error instanceof Error ? error.message : "No hemos podido importar el archivo.", "error");
    } finally {
      setBusy("idle");
    }
  }
  ...
  async function generateAIPreview() {
    if (!rows.length || !analysis) {
      showToast("Analiza un catálogo antes de generar preview IA.", "warning");
      return;
    }
    setBusy("aiPreview");
    try {
  ...
  async function createJob() {
    ...
    if (!analysis || !rows.length) {
      showToast("Analiza un catálogo antes de crear el job.", "warning");
      return;
    }
    if (rows.length > maxRowsPerJob) {
      showToast(`Este catálogo supera el límite de ${maxRowsPerJob.toLocaleString("es-ES")} filas por job. Divide el archivo en lotes.`, "warning");
      return;
    }
    if (creditDeficit > 0) {
    ...
    try {
      ...
      const normalizedCsv = importCatalog ? importedRowsToCsv(importCatalog.headers, importCatalog.rows) : csvText;
      ...
      const response = await fetch("/api/jobs/create", {
        ...
        body: JSON.stringify({
          ...
          importMetadata: importCatalog ? { sourceType: importCatalog.sourceType, originalFileName: importCatalog.fileName ?? file?.name ?? fileName, sheetName: importCatalog.sheetName, encoding: importCatalog.encoding, delimiter: importCatalog.delimiter, platformGuess: importCatalog.platformGuess?.platform, mappingConfidence: Math.round((importCatalog.mappingSuggestions.filter((item) => item.confidence > 0).length / Math.max(importCatalog.mappingSuggestions.length, 1)) * 100) / 100, warnings: importCatalog.warnings } : { sourceType: "csv", originalFileName: fileName },
        }),
      });
