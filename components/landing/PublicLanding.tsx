...
const creditPacks = [
  { credits: 10000, price: 9, ideal: "Primer diagnóstico o metadatos de un catálogo pequeño" },
  { credits: 25000, price: 19, ideal: "Tiendas pequeñas con optimización puntual" },
  { credits: 50000, price: 29, ideal: "Ecommerce en crecimiento con productos recurrentes" },
  { credits: 100000, price: 49, ideal: "Catálogos medianos y agencias pequeñas" },
  { credits: 250000, price: 99, ideal: "Lotes completos de hasta 1.000 productos" },
  { credits: 500000, price: 179, ideal: "Catálogos grandes con productos y categorías" },
  { credits: 1000000, price: 299, ideal: "Agencias, B2B industrial y migraciones" },
];

const monthlyPlans = [
  { name: "Free", price: "0 €", credits: "10.000 créditos demo", features: ["Diagnóstico CSV", "5 filas de preview", "Exportación limitada"], cta: "Empezar gratis" },
  { name: "Starter", price: "19 €/mes", credits: "50.000 créditos/mes", features: ["Productos SEO", "Metadatos", "Export CSV básico"], cta: "Elegir Starter" },
  { name: "Pro", price: "49 €/mes", credits: "250.000 créditos/mes", features: ["Categorías SEO", "Export Shopify/Prestashop/WooCommerce", "Score SEO", "Descargas"], cta: "Elegir Pro", featured: true },
  { name: "Agency", price: "149 €/mes", credits: "1.000.000 créditos/mes", features: ["Multi-proyecto", "Procesamiento prioritario", "Plantillas por sector", "Reportes"], cta: "Hablar para Agency" },
];
    ...
    <div className="min-h-screen bg-slate-50 text-slate-950">
      ...
      <main>
        ...
        <section id="precios" className="mx-auto max-w-7xl px-4 py-20 sm:px-6 lg:px-8">
          <div className="text-center"><Badge variant="ai">Pricing público</Badge><h2 className="mt-4 text-4xl font-black lg:text-5xl">Créditos o planes mensuales, sin complicaciones</h2></div><h3 className="mt-12 text-2xl font-black">Pago por créditos</h3><div className="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-4">{creditPacks.map((pack) => <Card key={pack.credits}><h4 className="text-xl font-black">{formatNumber(pack.credits)} créditos</h4><p className="mt-2 text-3xl font-black">{formatCurrency(pack.price)}</p><p className="mt-3 text-sm text-slate-600">~{formatNumber(Math.floor(pack.credits / 250))} productos completos · ~{formatNumber(Math.floor(pack.credits / 25))} metadatos · ~{formatNumber(Math.floor(pack.credits / 2500))} categorías.</p><p className="mt-3 text-sm font-semibold text-slate-500">Ideal para: {pack.ideal}</p><Button className="mt-5 w-full" href="/login?mode=register&intent=credits" variant="secondary">Comprar</Button></Card>)}</div><h3 className="mt-12 text-2xl font-black">Planes mensuales</h3><div className="mt-5 grid gap-5 md:grid-cols-2 xl:grid-cols-4">{monthlyPlans.map((plan) => <Card className={plan.featured ? "ring-2 ring-blue-500" : ""} key={plan.name} variant={plan.featured ? "gradient" : "default"}>{plan.featured && <Badge variant="ai">Recomendado</Badge>}<h4 className="mt-3 text-2xl font-black">{plan.name}</h4><p className="mt-3 text-4xl font-black">{plan.price}</p><p className="mt-2 font-semibold text-blue-700">{plan.credits}</p><ul className="mt-5 space-y-2 text-sm text-slate-600">{plan.features.map((feature) => <li key={feature}>✓ {feature}</li>)}</ul><Button className="mt-6 w-full" href="/login?mode=register&intent=plan" variant={plan.featured ? "primary" : "secondary"}>{plan.cta}</Button></Card>)}</div>
        </section>
