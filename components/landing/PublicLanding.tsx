...
const monthlyPlans = SUBSCRIPTION_PLANS.map((plan) => ({ name: plan.name, price: plan.price, products: `${plan.monthlyProducts.toLocaleString("es-ES")} productos estándar/mes`, equivalent: plan.equivalent, features: plan.features, cta: plan.id === "free" ? "Empezar gratis" : `Elegir ${plan.name}`, featured: plan.featured }));
