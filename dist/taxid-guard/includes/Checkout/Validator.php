...
class Validator
{
    ...
    public function validate_classic(): void
    {
        ...
        $this->apply_validation_result($this->validate($input, 'classic'), 'classic');
    }
    ...
    private function save_meta(\WC_Order $order): void
    {
        ...
        $context = isset($m['__tg_context']) ? (string) $m['__tg_context'] : 'runtime';
        $input = $m['__tg_input'] instanceof TaxIdInput ? $m['__tg_input'] : new TaxIdInput(! empty($m['is_company']), (string) ($m['tax_id'] ?? ''), (string) ($m['country'] ?? ''));
