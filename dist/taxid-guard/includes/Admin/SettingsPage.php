...
class SettingsPage
{
    ...
    public static function defaults(): array
    {
        return [
            ...
            'tg_store_ip_address' => 'hash',
            'tg_data_retention_days' => 365,
            ...
            'tg_delete_data_on_uninstall' => 'no',
    ...
    public function register_settings(): void
    {
        ...
        register_setting($group, 'tg_store_ip_address', ['sanitize_callback' => fn($v) => $this->sanitize_select($v, ['no', 'hash', 'yes'], 'hash')]);
        register_setting($group, 'tg_data_retention_days', ['sanitize_callback' => fn($v) => $this->sanitize_int_range($v, 0, 3650, 365)]);
        ...
        register_setting($group, 'tg_delete_data_on_uninstall', ['sanitize_callback' => [$this, 'sanitize_yes_no']]);
    ...
    private function render_section_general(): void
    {
        ...
        $this->checkbox_row('tg_delete_data_on_uninstall', __('Delete plugin data on uninstall', 'taxid-guard-for-woocommerce'), __('If enabled, options and cached validation data are removed on uninstall.', 'taxid-guard-for-woocommerce'));
        echo '</table>';
    }
    ...
    private function render_section_checkout(): void
    {
        ...
        $this->select_row('tg_store_ip_address', __('Store customer IP', 'taxid-guard-for-woocommerce'), ['no' => __('No', 'taxid-guard-for-woocommerce'), 'hash' => __('Hashed only', 'taxid-guard-for-woocommerce'), 'yes' => __('Full IP', 'taxid-guard-for-woocommerce')], '');
...
", $summaryText)) . '</textarea>';
    ...
    {
        echo '<details style="margin-top:16px"><summary><strong>' . esc_html__('Pro available', 'taxid-guard-for-woocommerce') . '</strong></summary>';
