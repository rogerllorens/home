...
export function evaluateQuality(input: CsvRow, output: Record<string, unknown>, options: { platform?: string; contentType?: string; promptVersion?: string; model?: string; fallbackUsed?: boolean } = {}): QualityAudit {
  ...
  const imageSeoScore = Number(output.image_seo_score ?? 0);
  const imageWarnings = split(Array.isArray(output.image_alt_warnings) ? output.image_alt_warnings.join(" | ") : String(output.image_alt_warnings ?? ""));
  const imageRecommendations = split(Array.isArray(output.image_alt_recommendations) ? output.image_alt_recommendations.join(" | ") : String(output.image_alt_recommendations ?? ""));
  const imageSource = String(output.image_alt_source ?? "none") as "existing" | "ai" | "fallback" | "none";
  const imageHumanReview = Boolean(output.image_alt_human_review_required) || imageWarnings.some((warning) => /claim|precio|promoci|review|revisi/i.test(warning));
  ...
  const nonBlocking = uniq([...metaTitleIssues, ...metaDescriptionIssues, ...slugIssues, ...keywordIssues, ...missingData.map((item) => `Dato faltante: ${item}`), ...platformWarnings, ...eeatWarnings, ...geoWarnings, ...imageWarnings.map((item) => `Image SEO: ${item}`)].filter(Boolean));
  ...
  const rankelia = score(92, blocking.length * 18 + nonBlocking.length * 3 + (imageSeoScore && imageSeoScore < 55 ? 6 : 0));
  ...
  return {
    ...
    image_alt_status: String(output.image_alt_status ?? (imageSeoScore >= 82 ? "good" : imageSeoScore ? "warning" : "missing")) as "good" | "warning" | "missing" | "generated" | "needs_review",
    image_alt_warnings: uniq(imageWarnings).join(" | "),
    image_alt_recommendations: uniq(imageRecommendations).join(" | "),
    image_alt_source: imageSource,
    image_alt_human_review_required: imageHumanReview,
    ...
    human_review_required: readiness !== "ready" || unsupportedClaims.length > 0 || options.fallbackUsed === true || imageHumanReview,
