...
export function buildProductSystemPrompt(tier: ModelTier) {
  ...
  return `${detail}\nGenera una ficha SEO ecommerce estructurada para producto. No inventes para sonar premium: si falta un dato, adviértelo. Incluye unsupported_claims, manual_review_notes, confidence_score y ready_to_export. No inventes reviews, ratings, certificaciones, compatibilidades, beneficios médicos, precio, stock ni descuentos.`;
}

export function buildProductUserPrompt(row: CsvRow, settings: AIProcessingSettings) {
  return `Genera JSON de producto con el schema exacto solicitado.\nConfiguración: ${JSON.stringify(settings)}\nFila CSV normalizada: ${JSON.stringify(row)}\nCampos requeridos: seo_product_name, keyword_principal, keywords_secundarias, keywords_long_tail, entidades_relacionadas, short_description, long_description_html, bullet_points, benefits, technical_features, use_cases, meta_title, meta_description, slug, faqs, schema_product_json, image_alt_texts, internal_link_suggestions, cta, seo_score, conversion_score, quality_warnings, data_needed, forbidden_claims_avoided, unsupported_claims, manual_review_notes, confidence_score, ready_to_export.\nMeta title aprox. 45-60 caracteres. Meta description aprox. 120-160. FAQ solo con respuestas soportadas por datos. Schema sin aggregateRating/review/availability si no existe. Devuelve solo JSON válido.`;
}
