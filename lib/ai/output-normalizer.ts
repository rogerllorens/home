...
function altListFromPartial(aiPartial: Partial<GenerationOutput>) {
  const raw = aiPartial.image_alt_texts;
...
function finalize(aiPartial: Partial<GenerationOutput>, row: CsvRow, settings: WorkerGenerationSettings, extraWarnings: string[] = []) {
  ...
  const baseSeed = { ...original, ...aiPartial, quality_level: quality, product_equivalent_used: calculateProductEquivalentUsed(1, quality), internal_credits_used: calculateJobCredits(1, { generationType: settings.generation_type || settings.generationType, qualityLevel: quality }), status: "completed" as const, error_message: "" } as GenerationOutput;
  const imageSeo = enrichImageSeo(row, { productName: baseSeed.seo_product_name || original.nombre_producto, brand: original.marca, category: original.categoria, sku: original.sku, keyword: baseSeed.keyword_principal, features: original.features }, altListFromPartial(aiPartial));
  ...
  const qualityWarnings = [...detectQualityWarnings(row, base), base.quality_warnings, base.image_alt_warnings?.join(" | "), base.image_alt_recommendations?.join(" | "), ...extraWarnings, audit.non_blocking_warnings, audit.blocking_issues].filter(Boolean).join(" | ");
...
export function normalizeProductOutput(ai: ProductAIOutput | MetadataAIOutput, row: CsvRow, settings: WorkerGenerationSettings, extraWarnings: string[] = []): GenerationOutput {
  ...
  const partial: Partial<GenerationOutput> = {
    ...
    image_alt_texts: join([...(ai.image_alt_texts ?? []), ...(("primary_image_alt" in ai && ai.primary_image_alt) ? [ai.primary_image_alt] : []), ...(("gallery_image_alts" in ai && ai.gallery_image_alts) ? ai.gallery_image_alts : [])]),
