...
import { enrichImageSeo } from "../quality/image-alt";
...
export type GenerationOutput = QualityAudit & {
  ...
  faqs: string; schema_product_json: string; image_alt_texts: string; primary_image_url?: string; gallery_image_urls?: string[]; existing_image_alt_texts?: string[]; primary_image_alt?: string; gallery_image_alts?: string[]; image_seo_score?: number; image_alt_status?: "good" | "warning" | "missing" | "generated" | "needs_review"; image_alt_warnings?: string[]; image_alt_recommendations?: string[]; image_data_quality?: "complete" | "partial" | "missing"; image_alt_source?: "existing" | "ai" | "fallback" | "none"; image_alt_keyword_used?: string | null; image_alt_human_review_required?: boolean; internal_link_suggestions: string; cta: string; seo_score: number; conversion_score: number;
...
export function sanitizeCsvCell(value: unknown) {
  ...
  if (/^[=+\-@\t\r\n]/.test(text) || /^[=+\-@]/.test(trimmedStart)) return `'${text}`;
  return text;
}
...
export function originalDataFromRow(row: CsvRow, mapping?: ColumnMapping) {
  return {
    ...
    image_url: pick(row, mapping, "imagen_url", firstValue(row, ["imagen_url", "image_url", "image", "Image Src", "Images", "Image URLs", "Image URL", "url imagen", "foto_url", "gallery_images", "additional_images"])) || "",
...
function finalizeOutput(row: CsvRow, settings: WorkerGenerationSettings, partial: Omit<Partial<GenerationOutput>, keyof QualityAudit>) {
  ...
  const baseSeed = { ...original, ...partial, quality_level: quality } as unknown as GenerationOutput;
  const imageSeo = enrichImageSeo(row, { productName: baseSeed.seo_product_name || original.nombre_producto, brand: original.marca, category: original.categoria, sku: original.sku, keyword: baseSeed.keyword_principal, features: original.features }, splitList(baseSeed.image_alt_texts || ""));
  const base = { ...baseSeed, ...imageSeo, image_url: imageSeo.primary_image_url || baseSeed.image_url } as GenerationOutput;
  ...
  const imageWarnings = [base.image_alt_warnings?.join(" | "), base.image_alt_recommendations?.join(" | ")].filter(Boolean).join(" | ");
  const qualityWarnings = [base.quality_warnings, imageWarnings, audit.non_blocking_warnings, audit.blocking_issues].filter(Boolean).join(" | ");
...
function shopifyRow(r: ExportRow) { return { Handle: r.slug, Title: r.seo_product_name, "Body (HTML)": r.long_description_html, Vendor: r.marca, "Product Category": r.categoria, Type: r.categoria, Tags: tags(r), Published: "FALSE", "Option1 Name": "Title", "Option1 Value": "Default Title", "Variant SKU": r.sku, "Variant Price": r.price, "Image Src": r.primary_image_url ?? r.image_url, "Image Alt Text": r.primary_image_alt ?? first(r.image_alt_texts), "SEO Title": r.meta_title, "SEO Description": r.meta_description, Status: "draft", "Rankelia Warnings": platformWarnings(r, "shopify"), ready_to_publish: r.ready_to_publish, human_review_required: r.human_review_required, rankelia_quality_score: r.rankelia_quality_score }; }
function wooRow(r: ExportRow) { return { Type: "simple", SKU: r.sku, Name: r.seo_product_name, Published: "0", "Short description": r.short_description, Description: r.long_description_html, "Regular price": r.price, Categories: r.categoria, Tags: tags(r), Images: [r.primary_image_url ?? r.image_url, ...splitList(listValue(r.gallery_image_urls))].filter(Boolean).join(","), "Meta: _rankelia_image_alt": r.primary_image_alt ?? first(r.image_alt_texts), "Meta: _rankelia_gallery_alts": listValue(r.gallery_image_alts), "Meta: _yoast_wpseo_title": r.meta_title, "Meta: _yoast_wpseo_metadesc": r.meta_description, "Rankelia Warnings": platformWarnings(r, "woocommerce"), ready_to_publish: r.ready_to_publish, human_review_required: r.human_review_required, rankelia_quality_score: r.rankelia_quality_score }; }
function prestaRow(r: ExportRow) { return { ID: "", Name: r.seo_product_name, Categories: r.categoria, "Price tax excluded": r.price, Reference: r.sku, "Short description": r.short_description, Description: r.long_description_html, "Meta title": r.meta_title, "Meta keywords": r.keywords_secundarias, "Meta description": r.meta_description, "URL rewritten": r.slug, "Image URLs": [r.primary_image_url ?? r.image_url, ...splitList(listValue(r.gallery_image_urls))].filter(Boolean).join(","), "Rankelia Image Alt Texts": [r.primary_image_alt ?? first(r.image_alt_texts), ...splitList(listValue(r.gallery_image_alts))].filter(Boolean).join(" | "), Active: "0", "Rankelia Warnings": platformWarnings(r, "prestashop"), ready_to_publish: r.ready_to_publish, human_review_required: r.human_review_required, rankelia_quality_score: r.rankelia_quality_score }; }
...
export function buildOutputHTML(results: GenerationOutput[], job: { id: string; original_filename?: string | null; platform?: string }) {
  ...
  const avgImage = Math.round(results.reduce((s, r) => s + (r.image_seo_score ?? 0), 0) / Math.max(results.length, 1));
  ...
  return `<!doctype html><html lang="es"><head><meta charset="utf-8"><title>Rankelia resultados ${safeJobId}</title><style>body{font-family:system-ui;background:#f8fafc;color:#0f172a;padding:32px}.card{background:#fff;border:1px solid #e2e8f0;border-radius:20px;padding:24px;margin:18px 0}.score{color:#10b981;font-weight:900}.warn{color:#b45309}</style></head><body><h1>Resultados SEO Rankelia</h1><p>Archivo: ${safeFile} · Plataforma: ${safePlatform} · Revisar antes de publicar.</p><p class="score">Confidence medio ${avgConfidence}/100 · E-E-A-T ${avgEEAT}/100 · GEO/AI readiness ${avgGeo}/100 · Image SEO ${avgImage}/100</p>${results.map((r) => `<article class="card"><h2>${escapeHtml(r.seo_product_name)}</h2><p>${escapeHtml(r.short_description)}</p><pre>${escapeHtml(r.bullet_points)}</pre><p><b>Descripción larga saneada:</b></p><div>${sanitizeHtmlFragment(r.long_description_html)}</div><p><b>Meta title:</b> ${escapeHtml(r.meta_title)} (${escapeHtml(r.meta_title_status)})</p><p><b>Meta description:</b> ${escapeHtml(r.meta_description)} (${escapeHtml(r.meta_description_status)})</p><p><b>Image SEO:</b> ${escapeHtml(r.image_seo_score ?? 0)}/100 · ALT: ${escapeHtml(r.primary_image_alt ?? first(r.image_alt_texts))} · source ${escapeHtml(r.image_alt_source ?? "none")}</p><p class="score">SEO ${r.seo_score}/100 · Conversión ${r.conversion_score}/100 · Rankelia ${r.rankelia_quality_score}/100</p><p><b>Ready:</b> ${escapeHtml(r.ready_to_publish)} · Human review: ${r.human_review_required ? "sí" : "no"}</p><p class="warn">${escapeHtml(r.quality_warnings)}</p></article>`).join("")}</body></html>`;
}
...
export function buildReportTXT(job: { id: string; original_filename?: string | null; platform?: string; estimated_credits?: number }, results: GenerationOutput[], summary: { failed: number; warnings: number }) {
  ...
  const avgGeo = Math.round(results.reduce((s, r) => s + r.geo_ai_readiness_score, 0) / Math.max(results.length, 1));
  const avgImage = Math.round(results.reduce((s, r) => s + (r.image_seo_score ?? 0), 0) / Math.max(results.length, 1));
  const imageDetected = results.filter((r) => r.primary_image_url || r.image_url).length;
  const imageGenerated = results.filter((r) => r.image_alt_source === "ai" || r.image_alt_source === "fallback").length;
  const imageExisting = results.filter((r) => r.image_alt_source === "existing").length;
  const imageReview = results.filter((r) => r.image_alt_human_review_required).length;
