@php
    ...
    $cookieNotice = config('candidboys.ui.cookie_notice_text') ?: __('ui.cookies.notice');
    $cookieAdsNotice = config('candidboys.ui.cookie_ads_notice_text') ?: __('ui.cookies.ads_notice');
@endphp
    ...
    <div class="fixed inset-x-0 bottom-0 z-50 border-t border-white/10 bg-slate-950/95 px-4 py-4 text-sm text-slate-200 backdrop-blur">
        <div class="mx-auto flex max-w-6xl flex-col gap-3 md:flex-row md:items-center md:justify-between">
            ...
            <button
                ...
                type="button"
                data-cookie-accept
                aria-label="{{ __('ui.cookies.accept_aria') }}"
            >
                {{ __('ui.cookies.accept') }}
            </button>
        </div>
        <script nonce="{{ $cspNonce ?? '' }}">
            document.addEventListener('DOMContentLoaded', () => {
                ...
                button.addEventListener('click', () => {
                    ...
                    document.cookie = `${@json($cookieName)}=1; Max-Age=${maxAge}; Path=/; SameSite=Lax${secureFlag}`;
