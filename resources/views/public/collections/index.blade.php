@php
    ...
    $pageTitle = __('ui.meta.collections_title', ['brand' => $brand]);
@endphp
...
<x-layouts.public title="{{ $pageTitle }}">
    @push('head')
        <x-seo-head
            title="{{ $pageTitle }}"
            description="{{ __('ui.meta.collections_description', ['brand' => $brand]) }}"
        ...
        @forelse ($collections as $collection)
            <a class="rounded-xl border border-white/10 bg-white/5 p-5 transition hover:border-red-500/60 hover:bg-white/10" href="{{ route('public.collections.show', $collection->slug) }}">
                <div class="flex items-center justify-between">
                    ...
                    <span class="text-xs text-slate-400">{{ __('ui.collections.video_count', ['count' => $collection->videos_count]) }}</span>
                </div>
                <p class="mt-2 text-sm text-slate-300">{{ \Illuminate\Support\Str::limit($collection->description, 140) }}</p>
            </a>
        @empty
            <p class="text-sm text-slate-400">{{ __('ui.collections.empty') }}</p>
        @endforelse
    </div>
</x-layouts.public>
