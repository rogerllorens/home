@php
    ...
    $pageTitle = __('ui.meta.reels_title', ['brand' => $brand]);
@endphp
...
<x-layouts.public title="{{ $pageTitle }}">
    @push('head')
        <x-seo-head
            title="{{ $pageTitle }}"
            description="{{ __('ui.meta.reels_description', ['brand' => $brand]) }}"
    ...
    <section class="space-y-6" id="reels-feed" data-next-page="{{ $nextPage }}">
        <div class="rounded-xl border border-white/10 bg-white/5 p-4 md:p-6">
            <h1 class="text-3xl font-semibold text-white">{{ __('ui.reels.title') }}</h1>
            <p class="mt-2 text-sm text-slate-300">{{ __('ui.reels.intro') }}</p>
        </div>

        <div class="space-y-8 snap-y snap-mandatory md:snap-none">
            @foreach ($videos as $video)
                ...
                @endphp
                <article class="reel-card relative min-h-screen snap-start rounded-2xl border border-white/10 bg-black/80 p-4 md:p-6" data-reel-card data-video-id="{{ $video->id }}" data-embed-url="{{ $video->embed_url }}" data-iframe-sandbox="{{ $iframeSandbox }}" data-iframe-allow="{{ $iframeAllow }}">
                    <header class="flex flex-wrap items-center justify-between gap-3">
                        <div>
                            ...
                            <a class="rounded-md border border-white/10 px-3 py-2 text-xs font-semibold text-slate-200 hover:border-white/30 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/40 focus-visible:ring-offset-2 focus-visible:ring-offset-slate-950" href="{{ route('public.video', ['slug' => \Illuminate\Support\Str::slug($video->seo_title ?: $video->title), 'id' => $video->id]) }}">
                    ...
                    <div class="mt-4 flex flex-col gap-4 lg:flex-row">
                        <div class="relative w-full overflow-hidden rounded-xl bg-black lg:flex-1">
                            <div class="aspect-[9/16] w-full sm:aspect-video">
                                <img src="{{ $thumbnail }}" alt="{{ $video->title }}" class="h-full w-full object-cover" loading="lazy" decoding="async" data-reel-thumb>
                            </div>
                            <button class="absolute inset-0 flex items-center justify-center bg-black/50 text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-red-400 focus-visible:ring-offset-2 focus-visible:ring-offset-slate-950" type="button" data-reel-play aria-label="{{ __('ui.reels.play') }}">
                                <span class="rounded-full bg-red-600 px-6 py-3 text-sm font-semibold">{{ __('ui.reels.play') }}</span>
                            </button>
                        ...
                        class="inline-flex items-center gap-2 rounded-full border border-rose-500/60 px-3 py-1 text-xs font-semibold text-rose-100 transition hover:border-rose-400 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-rose-400 focus-visible:ring-offset-2 focus-visible:ring-offset-slate-950 {{ $video->liked_by_device ? 'bg-rose-500/20' : '' }}"
                            ...
                            <button class="rounded-full border border-white/10 px-3 py-1 text-xs font-semibold text-slate-200 transition hover:border-white/30 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/40 focus-visible:ring-offset-2 focus-visible:ring-offset-slate-950" type="button">
                                {{ __('ui.reels.comments') }}
                            </button>
                            <a class="rounded-full bg-white/5 px-3 py-1 text-xs font-semibold text-slate-200 transition hover:bg-white/10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/40 focus-visible:ring-offset-2 focus-visible:ring-offset-slate-950" href="{{ route('public.video', ['slug' => \Illuminate\Support\Str::slug($video->seo_title ?: $video->title), 'id' => $video->id]) }}">
                                {{ __('ui.reels.open_video') }}
                            </a>
                        </div>
                    </div>
                </article>
    ...
    <script nonce="{{ $cspNonce ?? '' }}">
        document.addEventListener('DOMContentLoaded', () => {
            ...
            const labels = {
                ...
                openVideo: @json(__('ui.reels.open_video')),
                comments: @json(__('ui.reels.comments')),
                play: @json(__('ui.reels.play')),
            };
            ...
            const loadMore = async () => {
                ...
                payload.data.forEach((item) => {
                    ...
                    card.className = 'reel-card relative min-h-screen snap-start rounded-2xl border border-white/10 bg-black/80 p-4 md:p-6';
                    ...
                    card.innerHTML = `
                        <header class="flex flex-wrap items-center justify-between gap-3">
                            ...
                            <a class="rounded-md border border-white/10 px-3 py-2 text-xs font-semibold text-slate-200 hover:border-white/30 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/40 focus-visible:ring-offset-2 focus-visible:ring-offset-slate-950" href="/v/${item.slug}-${item.id}">
                                ${labels.viewDetails}
                            </a>
                        </header>
                        <div class="mt-4 flex flex-col gap-4 lg:flex-row">
                            <div class="relative w-full overflow-hidden rounded-xl bg-black lg:flex-1">
                                ...
                                <button class="absolute inset-0 flex items-center justify-center bg-black/50 text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-red-400 focus-visible:ring-offset-2 focus-visible:ring-offset-slate-950" type="button" data-reel-play aria-label="${labels.play}">
                                    <span class="rounded-full bg-red-600 px-6 py-3 text-sm font-semibold">${labels.play}</span>
                                </button>
                                <div class="hidden absolute inset-0 h-full w-full bg-black" data-reel-embed></div>
                            </div>
                            <div class="flex items-start justify-between gap-4 text-sm text-slate-200 lg:w-40 lg:flex-col">
                                <button type="button" class="inline-flex items-center gap-2 rounded-full border border-rose-500/60 px-3 py-1 text-xs font-semibold text-rose-100 transition hover:border-rose-400 ${item.liked ? 'bg-rose-500/20' : ''}" data-like-button data-video-id="${item.id}" data-liked="${item.liked}" aria-pressed="${item.liked}" aria-label="Like video">
                                    <svg class="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true">
                                        <path d="M12 21s-6.7-4.35-9.33-7.5C.6 10.9 1.2 7.8 3.6 6.3c1.9-1.2 4.4-.9 6 1 1.6-1.9 4.1-2.2 6-1 2.4 1.5 3 4.6.93 7.2C18.7 16.65 12 21 12 21z"/>
                                    </svg>
                                ...
                                <a class="rounded-full bg-white/5 px-3 py-1 text-xs font-semibold text-slate-200 transition hover:bg-white/10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/40 focus-visible:ring-offset-2 focus-visible:ring-offset-slate-950" href="/v/${item.slug}-${item.id}">
                                    ${labels.openVideo}
                                </a>
                            </div>
                        </div>
