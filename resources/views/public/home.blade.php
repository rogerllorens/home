...
@else
    ...
    @if ($temporalSection->isNotEmpty())
        <section class="mb-10">
            <div class="mb-4 space-y-1">
                <p class="text-xs font-semibold uppercase tracking-wide text-slate-400">{{ __('ui.home.temporal_label') }}</p>
                <h2 class="text-xl font-semibold text-white">{{ __('ui.home.temporal_title') }}</h2>
                <p class="text-sm text-slate-400">{{ __('ui.home.temporal_subtitle') }}</p>
            </div>
            <x-video-grid>
                @foreach ($temporalSection as $video)
                    <x-video-card :video="$video" />
                @endforeach
            </x-video-grid>
        </section>
