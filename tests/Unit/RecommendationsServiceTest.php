...
class RecommendationsServiceTest extends TestCase
{
    ...
    public function test_recommendations_apply_temporal_preferences(): void
    {
        $deviceHash = 'device-temporal';
        ...
        $temporalMatch = Video::factory()->create([
            'status' => VideoStatus::Published,
            'category_slug' => 'massage',
            'raw_tags' => ['relax'],
        ]);

        app()->instance(TemporalPatternsService::class, new class {
            public function preferredForDevice(string $deviceHash, $now = null): array
            {
                return ['categories' => ['massage'], 'tags' => []];
            }
        });
        ...
        $this->assertTrue($recommended->contains('id', $temporalMatch->id));
    }
}
