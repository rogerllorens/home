...
async function processJob(job: Job) {
  ...
  try {
    ...
    const imageStats = { detected: 0, existing: 0, generated: 0, missing: 0, review: 0, scoreTotal: 0 };
    let processed = 0;
    let failed = 0;
    ...
    for (const [index, row] of rows.entries()) {
      try {
        ...
        results.push(output);
        imageStats.detected += output.primary_image_url || output.image_url ? 1 : 0;
        imageStats.existing += output.image_alt_source === "existing" ? 1 : 0;
        imageStats.generated += output.image_alt_source === "ai" || output.image_alt_source === "fallback" ? 1 : 0;
        imageStats.missing += output.image_alt_status === "missing" ? 1 : 0;
        imageStats.review += output.image_alt_human_review_required ? 1 : 0;
        imageStats.scoreTotal += output.image_seo_score ?? 0;
        processed += 1;
        ...
        if (output.image_alt_source === "fallback") await createJobLog(job.id, job.user_id, "warning", "image_alt_fallback_used", { row_index: index, image_seo_score: output.image_seo_score }, "ai");
        if (output.image_alt_human_review_required) await createJobLog(job.id, job.user_id, "warning", "image_alt_review_required", { row_index: index, warnings: output.image_alt_warnings }, "ai");
        const rowResult = await supabase.from("job_rows").upsert({ job_id: job.id, user_id: job.user_id, row_index: index, input_data: normalized, output_data: output, seo_score: output.seo_score, conversion_score: output.conversion_score, detected_issues: output.quality_warnings ? output.quality_warnings.split(" | ") : [], validation_status: output.quality_warnings ? "warning" : "valid", priority: output.quality_warnings ? "medium" : "low", status: "completed", error_message: null, ai_provider: aiResult.provider, ai_model: aiResult.model, prompt_version: aiResult.promptVersion, input_tokens_estimated: aiResult.inputTokens, output_tokens_estimated: aiResult.outputTokens, ai_cost_estimated: aiResult.cost, raw_ai_output: aiResult.rawAIOutput, validation_errors: aiResult.validationErrors, unsupported_claims: aiResult.unsupportedClaims, fallback_used: aiResult.fallbackUsed, generation_attempts: aiResult.generationAttempts, json_repaired: aiResult.jsonRepaired }, { onConflict: "job_id,row_index" }).select("id").single<{ id: string }>();
        if (rowResult.data?.id) {
          try {
            await createProposalFromJobRow(supabase, { userId: job.user_id, jobId: job.id, jobRowId: rowResult.data.id, rowIndex: index, originalData: normalized, outputData: output, metadata: { provider: aiResult.provider, model: aiResult.model, promptVersion: aiResult.promptVersion, fallbackUsed: aiResult.fallbackUsed, cost: aiResult.cost, generationEngine: settings.generation_engine } });
          } catch (proposalError) {
            await createJobLog(job.id, job.user_id, "warning", "proposal_creation_failed", { row_index: index, error: proposalError instanceof Error ? proposalError.message : "unknown" });
          }
        }
      } catch (error) {
        failed += 1;
    ...
    const avgImageSeoScore = Math.round(imageStats.scoreTotal / Math.max(results.length, 1));
...
Image SEO:
Images detected: ${imageStats.detected}
Existing ALT preserved: ${imageStats.existing}
ALT generated/suggested: ${imageStats.generated}
ALT missing: ${imageStats.missing}
ALT review required: ${imageStats.review}
Average image SEO score: ${avgImageSeoScore}
`;
    ...
    const email = await getUserEmailForJob(job.user_id);
    await createJobLog(job.id, job.user_id, "info", "image_alt_generation_completed", { images_detected: imageStats.detected, image_alts_existing: imageStats.existing, image_alts_generated: imageStats.generated, image_alts_missing: imageStats.missing, avg_image_seo_score: avgImageSeoScore, image_alt_review_required_count: imageStats.review });
    await createJobLog(job.id, job.user_id, "info", "Job completed", { processed, failed, averageScore, downloads: downloads.length, proposals_created: processed, pending_review_count: processed, email: email ? "queued" : "missing", ai: aiStats, image: { ...imageStats, avgImageSeoScore } });
