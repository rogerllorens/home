...
## Image SEO y ALT text

Rankelia Generic CSV añade campos de imagen como `primary_image_url`, `gallery_image_urls`, `primary_image_alt`, `gallery_image_alts`, `existing_image_alt_texts`, `image_alt_source`, `image_seo_score`, `image_alt_status`, `image_alt_warnings`, `image_alt_recommendations` y `image_alt_human_review_required`.

Shopify recibe `Image Src` e `Image Alt Text` para la imagen principal. WooCommerce y PrestaShop reciben URLs de imagen y columnas Rankelia orientativas de ALT; pueden requerir plugin, mapeo manual o ajustes del importador para aplicar ALT por imagen.

## Approved-only exports

The approved export flow generates Rankelia Generic, platform CSV, HTML report and TXT summary using only approved proposal versions. Pending or needs-review proposals are excluded by design.
