...
class HomeController extends Controller
{
    public function __invoke(Request $request)
    {
        ...
        if ($deviceHash) {
            ...
            $temporal = app(TemporalPatternsService::class)->preferredForDevice($deviceHash);
            $temporalSection = $this->temporalSection($temporal);
        }
    ...
    private function temporalSection(array $temporal)
    {
        $categories = $temporal['categories'] ?? [];
        $tags = $temporal['tags'] ?? [];
        ...
        return $query->orderByDesc('published_at')->take(12)->get();
    }
}
